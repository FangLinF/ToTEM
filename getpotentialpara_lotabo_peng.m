...
% 20210223 目前lotabo只有给出原子的特性，所以这里我们使用的是离子特性；如果要使用彭老师的，需要这里转换一下
% 增加flag，这样转换方便一些，代码的利用率高一些
if flag=='p'|| flag=='n'  %彭或加修正
     [PengPara, PengIon]=ABSFpara;  %提出所有的参数 %so, only peng's parameter and cannot be used further
elseif flag=='l' %lotabo
     [LotaboPara, LotaboIon]=Lotabo_para;  %get all parameters of lotabo  %20210223 now ion is empty
%else flag == 'k' %kirkland  %长度是12，导入不进来，修改要比较大算了 
end
if flag=='l'   %如果是使用lotabo的参数，把十个离子的参数赋值到peng的矩阵，以便后面使用
    PengPara=LotaboPara;
end
...
if ~isempty(b)
    ...
    all_nuclear(:,7:16) =  [PengPara(all_nuclear(:,1), 1), PengPara(all_nuclear(:,1), 6), PengPara(all_nuclear(:,1), 2), ...
                        ...
                        PengPara(all_nuclear(:,1), 5), PengPara(all_nuclear(:,1), 10)]; %每个原子的peng的散射参数
end
